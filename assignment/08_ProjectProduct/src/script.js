...
// alert("welcome to Group 5 store");
    ...
    () => {
      if(product[n].total !== 0){
        ...
        if (productcart[m].id === product[n].productId ) {
          ...
          if (product[n].total > 0) {
            document.getElementById("p" + product[n].productId).innerHTML = "Total : " + --product[n].total + " piece";
          }
          return;
        } 
      }
      productcart.push({ id: product[n].productId, name: product[n].productName ,price: product[n].price ,amount: 1 });
      document.getElementById("cart").innerHTML = "cart : " + amountInCart();
